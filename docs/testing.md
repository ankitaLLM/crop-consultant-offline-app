...
### Scenario A: Zero-Network & Downloaded Offline Street Map
...
2. Without a Google Maps API key configured in `src/config.js`, verify:
   ...
   - A distinct banner appears across the map header: **"Offline street map · roads, fields, and observations"**.
   - Nearby roads and road names remain visible beneath field boundaries without a network connection.
